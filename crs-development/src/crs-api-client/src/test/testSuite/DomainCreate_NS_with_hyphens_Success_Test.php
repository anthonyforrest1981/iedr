...
class DomainReg12 extends IEDR_BPR_Test

    //This test was written to verify that the API was accepting hyphens in the NS name as this was a previous bug.

{
     ...
     */
    public function testDomainCreateNSHyphen() {

        $command = new DomainCreateCommand("new-example-000027.ie");
        $command->addAdmin("XBC189-IEDR");
        $command->addTech("XBC189-IEDR");
        $command->addNs(new DomainNs("ns1.ds-dsd.ie"));
        $command->addNs(new DomainNs("ns2.ye"));
    ...
    /**
     * @depends testDomainCreateNSHyphen
     * @group CreateTicketSuccessDbTest
     */
    public function testDomainCreateNSHyphenDbTest() {

        $arr = DBUtils::getTicketData("new-example-000027.ie", "1");
        ...
        $this->assertEquals($dom, 'new-example-000027.ie');
        $this->assertEquals($type, 'Registration');
        $this->assertEquals($owner, 'XBC189-IEDR');
        $this->assertEquals($holder, 'mary gun');
        ...
        /*$this->assertEquals($ns1, 'ns1.ds-dsd.ie');
        $this->assertEquals($ns2, 'ns2.yy');
