...
public class MyValidationEventHandler implements ValidationEventHandler {
    ...
    public boolean handleEvent(ValidationEvent event) {
        this.schemaErrorMessage = String.format("Severity: %s\nMessage: %s\nLocation: %s\nException: %s",
                formatSeverity(event.getSeverity()), event.getMessage(), formatLocation(event.getLocator()),
    ...
    protected String formatLocation(ValidationEventLocator locator) {
        return String.format("URL: %s\nNode: %s\nObject: %s\nLine: %s Column: %s Offset: %s", locator.getURL(),
                locator.getNode(), locator.getObject(), locator.getLineNumber(), locator.getColumnNumber(),
                locator.getOffset());
    }
