...
public class DomainTransferCommandHandler extends AbstractDomainCommandHandler<TransferType> {
    ...
    public ResponseType handle(AuthData auth, TransferType command, ValidationCallback callback)
        ...
        if (isEmptyTechContactElement(command) || isAllDefaultsUse(command)) {
            try {
                defaults = getNicHandleAppService().getDefaults(auth.getUser());
    ...
    private TransferRequestVO prepareTransferRequest(TransferType command, ResellerDefaults defaults)
            throws DomainNotFoundException, NameserverNameSyntaxException {
        ...
        boolean charity = isCharityDomain(command.getName());
        transferReq.setCharity(charity);
        if (!charity && command.getPeriod() != null) {
        ...
        if (command.getPayFromDeposit() != null) {
            transferReq.setAutorenewMode(command.getPayFromDeposit().isAutorenewEnabled());
        }

        transferReq.setTicketSource(TicketSource.API);
    ...
    private void useAllDefaults(TransferRequestVO transferRequest, ResellerDefaults defaults) {
        ...
        List<Nameserver> nameservers = new ArrayList<>(defaultNameservers.size());
        for (String nsName : defaultNameservers) {
            nameservers.add(new Nameserver(nsName, null, null));
        }
        transferRequest.setNameservers(nameservers);
    }

    private void useDefaultTechContactAndCommandNs(TransferType command, TransferRequestVO transferRequest,
            ResellerDefaults defaults) throws NameserverNameSyntaxException {
        transferRequest.setTechContactNicHandle(defaults.getTechContactId());
