...
import static pl.nask.crs.iedrapi.CommandTypeEnum.*;
...
public class CommandExecutorImpl implements CommandExecutor {
    ...
    //            throws CommandExecutorFailedCommand {
    //
    ...
    @Transactional(rollbackFor = Exception.class, propagation = Propagation.NESTED)
    @Override
    public ResponseType execute(AuthData auth, CommandType command, ValidationCallback callback)
    ...
    public ResponseType doExecute(AuthData auth, CommandType command, ValidationCallback callback)
    ...
    private ResponseType handle(AuthData auth, ReadWriteType cmd, CommandTypeEnum cmdType, ValidationCallback callback)
            throws AccessDeniedException, IedrApiException {
        JAXBElement jaxbCmd = (JAXBElement) cmd.getAny();
        APICommandHandler handler = handlerRegistry.getHandler(cmdType, jaxbCmd.getName().getLocalPart(), jaxbCmd
                .getValue().getClass());

        if (handler == null) {
            log.error("Cannot find handler for parameters: cmdType=" + cmdType + ", cmdName="
                    + jaxbCmd.getName().getLocalPart() + ", class=" + jaxbCmd.getValue().getClass());
            throw new UnimplementedCommand();
        }
