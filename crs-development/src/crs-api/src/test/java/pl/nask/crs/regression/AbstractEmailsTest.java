...
public abstract class AbstractEmailsTest extends AbstractTransactionalTestNGSpringContextTests {
    ...
    protected void createExpectations(final int templateId, final Set<ParameterNameEnum> populatedParams)
            throws Exception {
        new NonStrictExpectations() {
            {
                ...
                result = new Delegate<Email>() {
                    ...
                    private void assertNotEmpty(EmailParameters params, ParameterNameEnum name) {
                        AssertJUnit.assertTrue("params should contain " + name,
                                params.getParameterNames().contains(name));
                        AssertJUnit.assertFalse("param should not be empty: " + name,
                                Validator.isEmpty(params.getParameterValue(name.getName(), false)));
                        AssertJUnit.assertFalse("param should not be replaced with param's name: " + name,
                                Validator.isEqual(name.getName(), params.getParameterValue(name.getName(), false)));
    ...
    protected void createExpectations(final int templateId, final Map<ParameterNameEnum, String> populatedValues)
            throws Exception {
        createExpectations(templateId, populatedValues, 1, 100);
    }

    protected void createExpectations(final int templateId, final Map<ParameterNameEnum, String> populatedValues,
            final int minTimesExpected, final int maxTimesExpected) throws Exception {
        new NonStrictExpectations() {
            {
                if (templateId != -1) {
                    emailTemplateSender.sendEmail(templateId, withInstanceOf(EmailParameters.class));
                ...
                result = new Delegate<Email>() {
                    ...
                    private void assertEqual(EmailParameters params, ParameterNameEnum name, String value) {
                        AssertJUnit.assertTrue("params should contain " + name,
                                params.getParameterNames().contains(name));
                        AssertJUnit.assertEquals("param should be replaced with right value: " + name, value,
                                params.getParameterValue(name.getName(), false));
                        System.out.println(name + " : " + params.getParameterValue(name.getName(), false));
                    }
                };
                minTimes = minTimesExpected;
                maxTimes = maxTimesExpected;
            }
        };
    }
