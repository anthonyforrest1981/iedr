 ...
 *
 * When promoting a charity ticket to a domain, the domains renewal date is set to today. As there is no payment associated with this type of registration, the D_Ren_Dt should be set to +1 Year
 *
 * Disclaimer: rolling a renewal date is being made with a proper DSM action triggered by an event.
 *
...
public class CharityRegistrationTest extends AbstractTransactionalTestNGSpringContextTests {
    ...
    @Resource
    TriplePassSupportService triplePassService;
    ...
    public void testPromoteTicketToDomain() throws Exception {
        ...
        ticketAppService.checkOut(adminUser, ticketId);
        Ticket ticket = ticketAppService.getTicketForDomain(adminUser, domainName);
        ticketAppService.accept(adminUser, ticketId, ticket.getOperation(), "accept");

        triplePassService.triplePass(adminUser, ticketId);

        ExtendedDomainInfo d = domainAppService.view(user, domainName);
        Date renDate = d.getDomain().getRenewalDate();
        Date crDate = d.getDomain().getRegistrationDate();

        AssertJUnit.assertEquals("Renewal date", renDate, DateUtils.truncate(DateUtils.addYears(crDate, 1), Calendar.DATE));
    }
}
