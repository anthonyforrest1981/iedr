...
public class EmailTemplateServiceImpl implements EmailTemplateService {
    ...
    public EmailTemplateServiceImpl(EmailTemplateDAO dao, EmailGroupDAO groupDao,
            HistoricalEmailTemplateDAO historicalDao, EmailDisablerService disablerService, EmailTemplateSender sender) {
        this.emailDao = dao;
    ...
    @Override
    public LimitedSearchResult<EmailTemplate> findEmailTemplate(EmailTemplateSearchCriteria criteria, long offset,
    ...
    public EmailTemplate save(EmailTemplate template, OpInfo op) {
        ...
        template = changeAllowedFields(dbTemplate, template);
        if (areDifferent(template, dbTemplate)) {
            ...
            HistoricalEmailTemplateKey key = historicalTemplateDao.create(template, new Date(), op.getActorName());
            emailDao.updateUsingHistory(key.getChangeId());
    ...
    private EmailTemplate changeAllowedFields(EmailTemplate dbTemplate, EmailTemplate template) {
        EmailTemplate resultTemplate = new EmailTemplate(dbTemplate);
        resultTemplate.setSendReason(template.getSendReason());
        resultTemplate.setGroup(template.getGroup());
        resultTemplate.setSuppressible(template.isSuppressible());
        resultTemplate.setNonSuppressibleReason(template.getNonSuppressibleReason());
    ...
    public void onGroupDeleted(EmailGroup group, OpInfo op) {
        ...
        for (EmailTemplate template : affected) {
            template.setGroup(null);
            HistoricalEmailTemplateKey key = historicalTemplateDao.create(template, changeDate, actorName);
            emailDao.updateUsingHistory(key.getChangeId());
        }
    }

    private boolean areDifferent(EmailTemplate a, EmailTemplate b) {
        return getGroupId(a) != getGroupId(b) || a.isSuppressible() != b.isSuppressible()
    ...
    private String makeInfo(EmailTemplate curr, EmailTemplate prev) {
        ...
        if (getGroupId(curr) != getGroupId(prev)) {
            builder.append("Group changed from \"").append(getGroupName(prev)).append("\" to \"")
                    .append(getGroupName(curr)).append("\"\n");
        }
        if (prev.isSuppressible() != curr.isSuppressible()) {
            builder.append("Is suppressible from \"").append(prev.isSuppressible() ? "Y" : "N").append("\" to \"")
                    .append(curr.isSuppressible() ? "Y" : "N").append("\"\n");
        }
        ...
        if (!prevSendReason.equals(currSendReason)) {
            builder.append("Send reason from \"").append(prevSendReason).append("\" to \"").append(currSendReason)
                    .append("\"\n");
        }
        ...
        if (!prevNonSuppressibleReason.equals(currNonSuppressibleReason)) {
            builder.append("Non suppressible reason from \"").append(prevNonSuppressibleReason).append("\" to \"")
                    .append(currNonSuppressibleReason).append("\"\n");
        }
