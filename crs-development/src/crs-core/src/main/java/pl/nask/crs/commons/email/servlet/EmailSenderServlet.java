...
public class EmailSenderServlet extends HttpServlet {
    ...
    private int getWorkerThreadsCount() {
        ...
        if (threads != null) {
            try {
                ...
                LOGGER.warn("Illegal value provided in system property " + key + ": " + threads + ". Using "
                        + maxThreads + " as the worker pool size");
            }
        }
    ...
    private synchronized String getSenderThreadInfo() {
        ...
        return String.format("Email sender thread is %s with %s emails in the queue. Configuration: %s.", running,
