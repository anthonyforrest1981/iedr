...
public class Group implements Permission {
     ...
     *
     * @param query the queried permission
     *
     ...
     */
    public boolean hasPermission(PermissionQuery query) throws PermissionDeniedException {
        PermissionDeniedException holdException = null;
        for (Permission hold : permissions) {
            try {
                if (hold.implies(query)) {
                    return true;
                }
    ...
    private void logException(PermissionQuery query, Permission hold, Exception e) {
        log.warn("Exception while checking permission " + query + " (" + query.getName() + ") with " + hold + " ("
                + hold.getName() + ")");
