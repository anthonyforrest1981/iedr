...
public class IncomingDocumentsManagerImpl implements IncomingDocumentsManager {
    ...
    @Override
    public void markDocumentFileAsAssigned(DocumentFile documentFile, boolean fileOkToMove)
        ...
        if (!tempFile.exists())
            throw new NoSuchDocumentException("File: " + documentFile.getFileName() + " by path: "
        ...
        final String newFilename = DocumentFile.getNormalizedFilenameWithAddDate(documentFile.getFileName(),
                documentFile.getAddDate());
        ...
        try {
            ...
            throw new DocumentFileMovingException("Document file: " + documentFile.getFileName()
                    + " cannot be moved to assigned directory.", e);
        }
    }

    @Override
    public void markFileAs(DocumentFileType type, File file)
            throws NoSuchDirectoryException, NoSuchDocumentException, DocumentFileMovingException {
        if (!file.exists())
        ...
        if (!targetFile.exists()) {
            try {
                ...
                throw new DocumentFileMovingException("Document file: " + file.getAbsolutePath()
                        + " cannot be moved to assigned directory.", e);
            }
        } else {
            LOGGER.warn("Tried to mark file " + origFilename + " as " + type.name()
                    + " but the name is already taken, ignoring");
        }
    ...
    @Override
    public DocumentFile getDocumentFile(String fileName, DocumentFileType docFileType)
    ...
    public void deleteFile(DocumentFile documentFile)
            throws DocumentFileMovingException, NoSuchDocumentException, NoSuchDirectoryException {
        ...
        if (!file.exists() || !file.isFile()) {
            throw new DocumentFileMovingException("File: " + fileName
                    + " cannot be deleted. Doesn't exist or is a directory.");
        }
        if (!file.delete()) {
    ...
    public boolean documentFileExists(DocumentFile documentFile)
            throws NoSuchDirectoryException, WrongFaxFileExtensionException {
        Validator.assertNotNull(documentFile, "document file");
        String fileName = documentFile.getFileName();
    ...
    public File getDirectory(DocumentFileType documentFileType) {
        switch (documentFileType) {
            case UPLOAD_VIA_CONSOLE:
                return new File(consoleUploadDir);
            case UPLOAD_VIA_MAIL:
                return new File(mailUploadDir);
            default:
