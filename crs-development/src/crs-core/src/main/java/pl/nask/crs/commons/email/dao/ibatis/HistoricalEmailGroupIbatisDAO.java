...
public class HistoricalEmailGroupIbatisDAO extends
        GenericIBatisDAO<HistoricalObject<EmailGroup>, HistoricalEmailGroupKey> implements HistoricalEmailGroupDAO {
    ...
    @Override
    public HistoricalEmailGroupKey create(EmailGroup emailGroup, Date changeDate, String changedBy) {
        HistoricalObject<EmailGroup> dto = new HistoricalObject<>(emailGroup, changeDate, changedBy);
        this.create(dto);
        return new HistoricalEmailGroupKey(dto.getObject().getId(), dto.getChangeId());
    }
}
