...
public class InvoiceXMLExporter extends XmlExporter<InvoiceType> implements InvoiceExporter {
    ...
    public InvoiceXMLExporter(VatDAO vatDAO, ApplicationConfig applicationConfig, DomainPricingDAO domainPricingDAO,
            DomainSearchService domainSearchService, AccountHelperService accountHelperService,
            NicHandleSearchService nicHandleSearchService) throws JAXBException, SAXException {
    ...
    public void export(Invoice invoice) throws ExportException {
        ...
        InvoiceToXmlConverter converter = new InvoiceToXmlConverter(vatDictionary, new DomainPricingDictionary(
                domainPricingDAO), domainSearchService, accountHelperService, nicHandleSearchService, applicationConfig);
        ...
        OutputStream destination = destinationFor(invoice.getInvoiceNumber(), invoice.getInvoiceDate(),
                xmlInvoiceExportConfiguration);
        MessageDigest md;
        try {
        ...
        invoice.setMD5(new String(Hex.encodeHex(md.digest())));
        ...
        OutputStream abmDestination = destinationFor(invoice.getInvoiceNumber(), invoice.getInvoiceDate(),
                abmXmlInvoiceExportConfiguration);
        DigestOutputStream abmDos = new DigestOutputStream(abmDestination, md);

        serialize(jaxbInvoice, dos, abmDos);
    }

    private OutputStream destinationFor(String invoiceNumber, Date invoiceDate, ExportConfiguration exportConfiguration)
