...
import pl.nask.crs.payment.dao.ibatis.converters.ExtendedReservationConverter;
 ...
 */
public class ConvertingReservationDAO extends ConvertingGenericDAO<InternalReservation, Reservation, Long> implements
        ReservationDAO {

    InternalReservationIbatisDAO internalDAO;

    public ConvertingReservationDAO(InternalReservationIbatisDAO internalDAO,
            Converter<InternalReservation, Reservation> internalConverter) {
    ...
    @Override
    public LimitedSearchResult<Reservation> getReservations(SearchCriteria<Reservation> criteria, long offset,
            long limit, List<SortCriterion> sortBy) {
        LimitedSearchResult<InternalReservation> res = internalDAO.getReservations((SearchCriteria) criteria, offset,
                limit, sortBy);
        List<Reservation> reservations = getInternalConverter().to(res.getResults());
        return new LimitedSearchResult<>(criteria, res.getLimit(), res.getOffset(), reservations, res.getTotalResults());
    }

    @Override
    public LimitedSearchResult<ExtendedReservation> findExtended(ExtendedReservationSearchCriteria criteria, long offset,
            long limit, List<SortCriterion> sortBy) {
        LimitedSearchResult<InternalExtendedReservation> result = internalDAO.findExtended((SearchCriteria) criteria,
                offset, limit, sortBy);
        List<ExtendedReservation> list = new ExtendedReservationConverter().to(result.getResults());
        return new LimitedSearchResult<>(criteria, limit, offset, list, result.getTotalResults());
    }
