...
public class DepositConverter extends AbstractConverter<InternalDeposit, Deposit> {
    ...
    protected Deposit _to(InternalDeposit internalDeposit) {
        return new Deposit(internalDeposit.getNicHandleId(), internalDeposit.getNicHandleName(),
    ...
    protected InternalDeposit _from(Deposit deposit) {
        return new InternalDeposit(deposit.getNicHandleId(), deposit.getTransactionDate(), deposit.getOpenBal(),
                deposit.getCloseBal(), deposit.getTransactionAmount(), deposit.getTransactionType(),
                deposit.getOrderId(), deposit.getCorrectorNH(), deposit.getRemark());
    }
}
