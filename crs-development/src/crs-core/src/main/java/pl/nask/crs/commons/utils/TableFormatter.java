...
public class TableFormatter implements HtmlPrintable {
    ...
    public static ColumnFormat dateYMDFormat(int desiredSize) {
        int yearSize = desiredSize - 6; // 6 is the length of "-MM-DD" part of "    YYYY-MM-DD" string.
        String sizeAsString = "";
        if (yearSize > 0)
            sizeAsString = Integer.toString(yearSize);
        // The desired format is %1$tY-%1$tm-%1$td - i.e. get first argument (%1$)
        // and print year (tY), month (tm) and day (td) value of it. But in order to have correct
        // length we need to pad the year part. So the year part is forced to be (desiredSize - 6)
        // long, that way after adding "-MM-DD" (6 characters) whole string is desiredSize long.
        return new ColumnFormat("%1$" + sizeAsString + "tY-%1$tm-%1$td");
    }
