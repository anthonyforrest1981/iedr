 ...
 */
public class HistoricalNicHandleConverter extends
        AbstractConverter<InternalHistoricalNicHandle, HistoricalObject<NicHandle>> {
    ...
    protected HistoricalObject<NicHandle> _to(InternalHistoricalNicHandle src) {
        return new HistoricalObject<>(src.getChangeId(), nicHandleConverter.to(src), src.getHistChangeDate(),
                src.getChangedByNicHandle());
    }

    protected InternalHistoricalNicHandle _from(HistoricalObject<NicHandle> historicalNicHandle) {
        InternalNicHandle inh = nicHandleConverter.from(historicalNicHandle.getObject());
        return new InternalHistoricalNicHandle(inh, historicalNicHandle.getChangeId(),
                historicalNicHandle.getChangeDate(), historicalNicHandle.getChangedBy());
    }
