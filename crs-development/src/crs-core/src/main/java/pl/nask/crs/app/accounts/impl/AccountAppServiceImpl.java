...
public class AccountAppServiceImpl implements AccountAppService {
    ...
    public AccountAppServiceImpl(AccountSearchService accountSearchService, AccountService accountService,
            HistoricalAccountSearchService historicalAccountSearchService, NicHandleService nicHandleService,
            ArtificialUsersRegistry artificialUsersRegistry) {
    ...
    public LimitedSearchResult<Account> search(AuthenticatedUser user, AccountSearchCriteria criteria, long offset,
    ...
    @Override
    @Transactional(readOnly = true)
    public Account get(AuthenticatedUser user, long id) throws AccessDeniedException, AccountNotFoundException {
        validateLoggedIn(user);
    ...
    public void alterStatus(AuthenticatedUser user, long id, AccountStatus status, String hostmastersRemark)
        ...
        accountService.alterStatus(id, status, opInfo);
    }
    ...
    public Account save(AuthenticatedUser user, Account account, AccountWrapper wrapper, String hostmastersRemark)
        ...
        account.setBillingContact(new Contact(wrapper.getNewBillingContactId(), wrapper.getNewBillingContactName(),
                wrapper.getNewBillingContactEmail()));
        OpInfo opInfo = new OpInfo(user, hostmastersRemark);
        accountService.save(account, opInfo);
    ...
    public Account create(AuthenticatedUser user, CreateAccountContener createAccountContener, String hostmastersRemark)
        ...
        OpInfo opInfo = new OpInfo(user, hostmastersRemark);
        accountService.createAccount(createAccountContener, opInfo);
        return createAccountContener.getAccount();
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public NewAccount createDirectAccount(NewNicHandle nhw, String password, boolean useTfa)
        ...
        OpInfo opInfo = new OpInfo(portalUser, "Direct Account Creation");
        NewAccount nh = nicHandleService.createDirectAccount(nhw.getName(), nhw.getCompanyName(), nhw.getEmail(),
                nhw.getAddress(), nhw.getCountryId(), nhw.getCountyId(), nhw.getPhones(), nhw.getFaxes(), nhw.getVatNo(),
                opInfo, password, useTfa, false);
        return nh;
    }
