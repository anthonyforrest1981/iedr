...
<xsl:template match="/">
    <fo:root>
        ...
        <fo:page-sequence master-reference="A4" id="seq1" font-family="Helvetica,NotoSans,Unifont">
                ...
                <fo:table table-layout="auto" border-width="0mm" border-style="solid" font-size="7pt">
                    ...
                    <fo:table-body>
                        <fo:table-row>
                            ...
                            <fo:table-cell>
                                <fo:block><xsl:value-of select="invoice/iedr/name"/></fo:block>
                            ...
                            <fo:table-cell>
                                <fo:table>
                                    ...
                                    <fo:table-body>
                                        ...
                                        <fo:table-row>
                                            <fo:table-cell>
                                                <xsl:if test="invoice/account/vatNo">
                                                    <fo:block>VAT Number:</fo:block>
                                                </xsl:if>
                                            </fo:table-cell>
                                            <fo:table-cell text-align="right">
                                                <fo:block><xsl:value-of select="invoice/account/vatNo"/></fo:block>
                                            </fo:table-cell>
                                        </fo:table-row>
                                    </fo:table-body>
                                </fo:table>
            ...
            <fo:flow flow-name="xsl-region-body">
                ...
                <fo:table id="total" font-size="7pt" margin-top="5mm" font-weight="bold" color="#ffffff">
                    ...
                    <fo:table-body>
                        <xsl:if test="invoice/transactions/transaction/item/VATCode='B'">
                            <fo:table-row>
                                <fo:table-cell number-columns-spanned="4">
                                    <fo:block color="#000000" font-style="italic" font-size="6pt" font-weight="normal">The recipient is obliged to account for the VAT arising on this supply in accordance with Article 196 of Council Directive 2006/112/EC.</fo:block>
                                </fo:table-cell>
                            </fo:table-row>
                        </xsl:if>
                        <fo:table-row>
...
<xsl:template name="typeCell">
    ...
    <xsl:choose>
        ...
        <xsl:when test="$value = 'buy request'">
            <xsl:call-template name="mainCell">
                <xsl:with-param name="value">BuyR</xsl:with-param>
            </xsl:call-template>
        </xsl:when>
        <xsl:when test="$value = 'sell request'">
            <xsl:call-template name="mainCell">
                <xsl:with-param name="value">SellR</xsl:with-param>
            </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
            <xsl:call-template name="mainCell">
