...
public class VatDAOTest extends AbstractTest {
    ...
    public void getVatTest() {
        ...
        AssertJUnit.assertEquals(MoneyUtils.getScaledVatValue(0.214), vat.getVatRate());
    ...
    public void createTest() {
        ...
        Vat newVat = new Vat(-1, "A", date, MoneyUtils.getScaledVatValue(0.5));
        vatDAO.create(newVat);

        Vat fromDB = vatDAO.getCurrentVat("A", date);

        AssertJUnit.assertEquals(newVat.getVatRate(), fromDB.getVatRate());

        fromDB = vatDAO.get(newVat.getId());
        AssertJUnit.assertEquals(org.apache.commons.lang.time.DateUtils.truncate(newVat.getFromDate(), Calendar.DATE),
                fromDB.getFromDate());
    }
