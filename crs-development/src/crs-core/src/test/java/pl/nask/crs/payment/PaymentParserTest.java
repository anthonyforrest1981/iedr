...
public class PaymentParserTest {
    ...
    public Object[][] requestsToTest() {
        return new Object[][] {
                {"settle transaction", makePaymentRequestAuthenticatedInstance(TransactionType.TYPE_SETTLE),
                        "payments/settleTransactionRequest.xml"},
                {"cancel transaction", makePaymentRequestAuthenticatedInstance(TransactionType.TYPE_VOID),
                        "payments/cancelTransactionRequest.xml"},
    ...
    private ExtendedPaymentRequest makeCCPaymentRequest() {
        PaymentRequest paymentRequest = PaymentRequest.createPaymentRequestWithStandardCurrencyUnit("EUR",
                MoneyUtils.getRoundedAndScaledValue(1000), "cardNumber", "expDate", "holderName", "VISA", "111", 1);
        return ExtendedPaymentRequest.authorisationInstance("account", "merchantId", "password", paymentRequest);
    }

    private ExtendedPaymentRequest makeCCPaymentRequest2DigitCVN() {
        PaymentRequest paymentRequest = PaymentRequest.createPaymentRequestWithStandardCurrencyUnit("EUR",
                MoneyUtils.getRoundedAndScaledValue(1000), "cardNumber", "expDate", "holderName", "VISA", "011", 1);
        return ExtendedPaymentRequest.authorisationInstance("account", "merchantId", "password", paymentRequest);
    }
    ...
    private ExtendedPaymentRequest makePaymentRequestAuthenticatedInstance(TransactionType type) {
        return ExtendedPaymentRequest
                .authorisedInstance("account", "merchantId", "password", type, "authcode", "pasref", "orderId");
    }

    class MyDifferenceListener implements DifferenceListener {
        private Set<String> xpathsToSkip = new HashSet<String>(Arrays.asList("/request[1]/@timestamp",
                "/request[1]/md5hash[1]/text()[1]", "/request[1]/orderid[1]/text()[1]"));
        ...
        private boolean shouldBeSkipped(Difference difference) {
            return difference.getControlNodeDetail().getXpathLocation()
                    .equals(difference.getTestNodeDetail().getXpathLocation())
