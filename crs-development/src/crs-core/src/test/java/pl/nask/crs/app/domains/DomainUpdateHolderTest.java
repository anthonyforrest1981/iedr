...
public class DomainUpdateHolderTest extends AbstractTransactionalTestNGSpringContextTests {
    ...
    public void hostmasterLogIn() throws Exception {
        user = authenticationService.authenticate("AAA906-IEDR", "Passw0rd!", false, "127.0.0.1", false, null, false,
                "crs");
    }
    ...
    private Domain findDomainWithPendingReservation(boolean b) throws AccessDeniedException {
        ExtendedDomainSearchCriteria criteria = new ExtendedDomainSearchCriteria();
        LimitedSearchResult<ExtendedDomain> res = domainSearchService.findExtended(criteria, 0, 1000, null);
        for (ExtendedDomain d : res.getResults()) {
            if (d.getDsmState().getDomainHolderType() == DomainHolderType.Billable
                    && domainAppService.isEventValid(user, d.getName(), DsmEventName.SetCharity)) {
                if (b == d.hasPendingReservations()) {
                    return d;
                }
            }
        }
    ...
    public void changeShouldBePermittedIfThereAreNoPendingReservations() throws Exception {
        ...
        Assert.assertEquals(domain.getDsmState().getDomainHolderType(), DomainHolderType.Charity,
                "Holder type should be changed");
    }
