...
public class DaoConfigTest extends AbstractTest {
    ...
    public void testInsertStringValue() {
        testInsertEntry(new ImmutableConfigEntry("ke\u0308y", "va\u0308lue", ConfigValueType.STRING));
        ConfigEntry entry = dao.getEntry("ke\u0308y");
        Assert.assertEquals(entry.getKey(), "këy");
        Assert.assertEquals(entry.getValue(), "välue");
        entry = dao.getEntry("këy");
        Assert.assertEquals(entry.getKey(), "këy");
        Assert.assertEquals(entry.getValue(), "välue");
    }
    ...
    public void testUpdateEntry() {
        dao.updateEntry(new ImmutableConfigEntry("testEntry", "va\u0308lue2", ConfigValueType.STRING));
        testGetEntry("testEntry", "välue2", ConfigValueType.STRING, "välue2");
    }
