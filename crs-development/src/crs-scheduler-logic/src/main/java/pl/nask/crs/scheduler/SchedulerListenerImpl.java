...
public class SchedulerListenerImpl implements it.sauronsoftware.cron4j.SchedulerListener {
    ...
    private void sendErrorMessageFallback(String jobName, Exception e) {
        ...
        email.setSubject("Fatal error while sending notification about errors in job " + jobName
                + " - check the logs for the details");
        StringWriter writer = new StringWriter();
        PrintWriter pw = new PrintWriter(writer);
        e.printStackTrace(pw);
        email.setText("Fatal error while sending notification about errors in job " + jobName
                + " - check the logs for the details. \n\r " + e.getMessage() + "\n\r" + writer.toString());
