...
class MimeMailParser {
    ...
    public function getInlineContent() {
        ...
        foreach ($this->parts as $part) {
            ...
            if ($content_id !== FALSE) {
                $content[] = new MimeMailParser_attachment($this->getPartContentName($part), $this->getPartContentType($part), $this->getAttachmentStream($part), $this->getPartContentDisposition($part), $this->getPartHeaders($part));
            }
        }
        return $content;
    }
        ...
        foreach ($this->parts as $part) {
            ...
            if (in_array($disposition, $dispositions) === TRUE || $content_name !== FALSE) {
                if ($content_name === FALSE) {
                    $content_name = isset($part['disposition-filename']) ? $part['disposition-filename'] : md5(uniqid());
                }
                if ($disposition === FALSE && $content_name !== FALSE) {
                    $disposition = $dispositions[0];
                }
                $attachments[] = new MimeMailParser_attachment($content_name, $this->getPartContentType($part), $this->getAttachmentStream($part), $disposition, $this->getPartHeaders($part));
            }
        }
        return $attachments;
    }
