...
reallyflush();
$dirlist = array('yii','yii/framework','css','html','protected','protected/db','protected/widgets',
    'protected/components','protected/wsapi','protected/views','protected/tests','protected/controllers',
    'protected/models','protected/runtime','protected/config','superfish','jqGrid','jqGrid/themes','jqGrid/js','themes',
...
reallyflush();
$needed_exts = array('curl','gd','json','libxml','mysql','mysqli','pcre','shmop','soap','sysvmsg','sysvsem','sysvshm',
    'xmlrpc','xsl','zip','zlib');
...
if (!file_exists($ckdnsURL)) {
    echo '<li> .. ERROR : Ckdns does not exist please contact asd@iedr.ie </li>' . "\n";
} else {
    echo '<li> .. OK.</li>' . "\n";
}
echo '<li> Check if CKDNS is executable.</li>' . "\n";
if (!is_executable($ckdnsURL)) {
    echo '<li> .. ERROR : Ckdns is not executable please contact asd@iedr.ie </li>' . "\n";
} else {
    echo '<li> .. OK.</li>' . "\n";
}
reallyflush();
