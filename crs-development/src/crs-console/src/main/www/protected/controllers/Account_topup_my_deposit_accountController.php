 ...
 * @author Artur Kielak
 */
class Account_topup_my_deposit_accountController extends AuthOnlyBaseController {

    protected function findDeposit($model, &$result) {
        $retval = CRSPaymentAppService_service::depositFunds($result, $this->backend_errors, Yii::app()->user->authenticatedUser,
                $model->euros_amount, CreditCardFormSegmentModel::asWSAPICreditCardObject($model));
        if (!$retval) {
