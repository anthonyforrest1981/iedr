...
class DomainsController extends AuthOnlyBaseController {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (isset($_POST['Domains_RegNew'])) {
                ...
                if ($rn_model->validate()) {
                    ...
                    AuthOnlyBaseController::safeSerializeAndStoreObjInSession($rn_model->domain_names, 'validavailabledomains');
    ...
    public function actionDomainDetails() {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (isset($_POST['Domains_Creation_Details'])) {
                ...
                $uploaderModel->setAllowedDomains($model->domainlist);
                $valid = $uploaderModel->validate() && $valid;
                if ($valid) {
                    $createResult = $this->createDomains($model);
                    $dc_model = new Domains_Created($createResult, $model);
                    // createResult looks like this:
                    // array(
                    // 'domainname01.ie' => array("error" => "error message"),
                    // 'domainname02.ie' => array("ticket" => number_of_ticket)
                    // )
                    // I need all domains that have a "ticket" key in their subarray.
                    $successfully_created_domains = array_keys(array_filter($createResult, function ($res) {
                        return array_key_exists('ticket', $res);
                    }));
                    $uploaderModel->setAllowedDomains($successfully_created_domains);
                    $uploaderModel->forceFilterDisallowedDocumentsAndDomains();
    ...
    private function setSessionFormPost($post) {
        $_SESSION['adminCompanyName0'] = $post['domain_adminContacts_0_nicHandle'];
        $_SESSION['adminCountry0'] = $post['domain_adminContacts_0_countryName'];
        ...
        $_SESSION['adminCountry1'] = $post['domain_adminContacts_1_countryName'];
        ...
        $_SESSION['techCountry'] = $post['domain_techContacts_countryName'];
    ...
    private function setNicHandle($name, $result) {
        if (property_exists($result, 'nicHandle')) {
            $_SESSION["$name"] = $result->nicHandle;
        } else if (property_exists($result, 'nicHandleId')) {
            $_SESSION["$name"] = $result->nicHandleId;
        }
    }

    private function setCountry($name, $result) {
        if (property_exists($result, 'countryName')) {
            $_SESSION["$name"] = $result->countryName;
        } else if (property_exists($result, 'country')) {
            $_SESSION["$name"] = $result->country->name;
        }
    }

    private function setSessionAdmin($num, $result) {
        if (is_array($result)) {
            ...
            $this->setNicHandle("adminNicHandleId$num", $result[$num]);
            $this->setCountry("adminCountry$num", $result[$num]);
        } else {
            $_SESSION["adminCompanyName$num"] = $result->companyName;
            $_SESSION["adminEmail$num"] = $result->email;
            $_SESSION["adminName$num"] = $result->name;
            $this->setNicHandle("adminNicHandleId$num", $result);
            $this->setCountry("adminCountry$num", $result);
        }
    }

    private function getAdminFromSession() {
        return array(
            array('companyName' => isset($_SESSION['adminCompanyName0']) ? $_SESSION['adminCompanyName0'] : '',
                'countryName' => isset($_SESSION['adminCountry0']) ? $_SESSION['adminCountry0'] : '',
            ...
            array('companyName' => isset($_SESSION['adminCompanyName1']) ? $_SESSION['adminCompanyName1'] : '',
                'countryName' => isset($_SESSION['adminCountry1']) ? $_SESSION['adminCountry1'] : '',
    ...
    private function setSessionTech($result) {
        $_SESSION['techCompanyName'] = $result->companyName;
        $_SESSION['techEmail'] = $result->email;
        $_SESSION['techName'] = $result->name;
        $this->setNicHandle('techNicHandleId', $result);
        $this->setCountry('techCountry', $result);
    }

    private function setTechFromSession(&$tech) {
        if (isset($_SESSION['techCompanyName'])) {
            $tech->companyName = $_SESSION['techCompanyName'];
            $tech->countryName = $_SESSION['techCountry'];
    ...
    private function setSession($nicrole, $result, &$domainVO) {
        switch ($nicrole) {
    ...
    public function actionViewDomain() {
        ...
        $domainVO = null;
        if (Yii::app()->request->isPostRequest) {
            if (isset($_POST['ViewDomainModel'])) {
                ...
                try {
                    ...
                    $domainName = Utf8Validator::validateAndNormalizeUtf8($domainName);
                    switch ($this->getPostAction($_POST)) {
                        ...
                        case 'Modify Domain':
                            ...
                            for ($i = 0; $i < $_POST['ViewDomainModel']['domain_nameserver_count']; $i++) {
                                if (isset($_POST['ViewDomainModel']['domain_nameservers'][$i])) {
                                    $nameservers[$i] = new CRSCommonAppService_nameserverVO();
                                    $nameservers[$i]->name = $_POST['ViewDomainModel']['domain_nameservers'][$i];
                                    $nameservers[$i]->ipv4Address = $_POST['ViewDomainModel']['domain_ipv4Addresses'][$i];
                                    $nameservers[$i]->ipv6Address = $_POST['ViewDomainModel']['domain_ipv6Addresses'][$i];
                                }
                            }
                            $domainVO = $this->getDomainDetails($domainName, $error);
                            $renewalMode = $domainVO->domain->dsmState->renewalMode;
                            ...
                            if (Utility::isLoggedInAs($domainVO->domain->billingContacts->nicHandle)) {
                                $uploaderModel->attributes = $_POST;
                                $uploaderModel->setAllowedDomains($domainName);
                                $validationPassed = $uploaderModel->validate() && $validationPassed;
                            }
                            if ($validationPassed) {
                                $ticketId = CRSCommonAppService_service::modifyDomain($result, $this->backend_errors,
                                        $user, $domainName, $domainHolder, $adminContacts, $techContacts, $nameservers,
                                        $renewalMode, $hostmasterRemark);
                                if ($result != null) {
                                    if (count($this->backend_errors) == 0) {
                                        $stringMessage = "Your modification request for " . $domainName
                                                . " has been submitted and will be processed by our Registrations team shortly.";
                    ...
                    Yii::log("Domain name $domainName is not a valid utf8 string. Exact exception message: " . $e->getMessage(), CLogger::LEVEL_ERROR);
                    array_push($vd_model->message, "Domain $domainName does not exist");
                }
            }
            if (isset($_POST['NicSearchModel']) and isset($_POST['NicSearchModel']['returningData'])) {
                $returndata = $_POST['NicSearchModel']['returningData'];
                ...
                $domain = mb_split("id=", $returndata['returnurl']);
                $domainName = $domain[1];
            ...
            try {
                $modifiedNicHandle = Utf8Validator::validateAndNormalizeUtf8($modifiedNicHandle);
            } catch (Exception $e) {
                Yii::log("NicHandle $modifiedNicHandle decoded from serialized obj is not a valid utf8 string, cannot sensibly recover." . " Will reset to empty string. Exact exception message: " . $e->getMessage(), CLogger::LEVEL_ERROR);
                $modifiedNicHandle = '';
            }
            try {
                $domainName = Utf8Validator::validateAndNormalizeUtf8($domainName);
            } catch (Exception $e) {
                Yii::log("Domain name $domainName is not a valid utf8 string, cannot sensibly recover." . "Will reset to empty string. Exact exception message: " . $e->getMessage(), CLogger::LEVEL_ERROR);
                $domainName = '';
            }
        } else {
            if (isset($_GET['id'])) {
                try {
                    $domainName = Utf8Validator::validateAndNormalizeUtf8($_GET['id']);
                } catch (Exception $e) {
                    $dName = $_GET['id'];
                    Yii::log("DomainName $dName is invalid utf8, defaulting to empty domain name. Exact exception message: " . $e->getMessage(), CLogger::LEVEL_ERROR);
                    $domainName = '';
                }
            }
        }
        ...
        $vd_model->overrideModifiedFields($dd_model, Utility::isDirect(), $nicrole);
        if (!$validationPassed) {
    ...
    public function setRegRequestObject($model, &$registrationRequest) {
        ...
        $registrationRequest->adminContactNicHandles = array();
        if (isset($model->admin_contact_nic_1)) {
            $registrationRequest->adminContactNicHandles[] = $model->admin_contact_nic_1;
        }
        if (isset($model->admin_contact_nic_2) && !empty($model->admin_contact_nic_2)) {
            $registrationRequest->adminContactNicHandles[] = $model->admin_contact_nic_2;
        } else {
            $registrationRequest->adminContactNicHandles[] = null;
        }
        if (isset($model->tech_contact)) {
            $registrationRequest->techContactNicHandle = $model->tech_contact;
        }
        if (isset($model->nameserversCount) && isset($model->nameservers) && isset($model->ipv4Addresses) && isset($model->ipv6Addresses)) {
            for ($i = 0; $i < $model->nameserversCount; $i++) {
                $nameserver = new CRSCommonAppService_nameserverVO();
                $nameserver->name = $model->nameservers[$i];
                $nameserver->ipv4Address = $model->ipv4Addresses[$i];
                $nameserver->ipv6Address = $model->ipv6Addresses[$i];
    ...
    public function setPaymentRequestObject($model, &$creditCard, &$registrationRequest) {
        switch ($model->paymentType) {
            ...
            case 'CH':
                ...
                break;
            case 'ADP_AUTORENEW':
                $registrationRequest->autorenewMode = true;
                break;
            case 'ADP':
                // do nothing
                break;
        }
        Yii::log('Payment request created');
    }
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if (isset($_POST['DomainsTransferDetails'])) {
                if ($dd_model->validate()) {
                    ...
                    $creditCard = null;
                    $this->setPaymentRequestObject($dd_model, $creditCard, $transferRequest);
                    Yii::log('registration request = ' . print_r($transferRequest, true));
                    if ($dd_model->charityPaymentForced) {
                        $transferRequest->charity = true;
                    } else {
                    ...
                    if ($authorization) {
                        $result = null;
                        CRSCommonAppService_service::transferDomain($result, $this->backend_errors, Yii::app()->user->authenticatedUser, $transferRequest, $creditCard);
                        $model = new stdClass();
                        $model->ticket = $result;
    ...
    public function actionDomainsCreated() {
        if (AuthOnlyBaseController::isSavedInSession('domaincreateresults')) {
            ...
            $this->render('domainscreated', array('model' => $dd_model,
                'uploaderResult' => new UploaderResultModelWrapper($docs_model)));
        } else {
            $this->redirect('index.php?r=domains/regnew', true);
        }
    }
     ...
     *
     * @param Domains_Creation_Details $model
     *            Domains_Creation_Details model, including domain list
    ...
    protected function createDomains($model) {
        ...
        $registrationRequest->domainOwnerTypeId = $model->ownerType;
        $registrationRequest->domainHolder = $model->holder; // type=xs:string, (NOT NULL), min=0
        $registrationRequest->adminContactNicHandles = array($model->admin_contact_nic_1, $model->admin_contact_nic_2); // type=xs:string, (NOT NULL), min=0
        ...
        $registrationRequest->requestersRemark = $model->getRemarks(); // calculated to include charitycode if required
        $registrationRequest->period = $reg_period_years;
        $creditCard = null;
        $this->setPaymentRequestObject($model, $creditCard, $registrationRequest);
        Yii::log('registrationRequest= ' . print_r($registrationRequest, true), 'debug', ' $creditCard ');
        foreach ($model->domainlist as $k => $domain) {
            $results = null;
            $registrationRequest->domainName = $domain;
            $this->fillIpAddresses($registrationRequest, $domain);
            CRSCommonAppService_service::registerDomain($result, $this->backend_errors, Yii::app()->user->authenticatedUser, $registrationRequest, $creditCard);
            if (count($this->backend_errors) == 0)
     ...
     */
    protected function fillIpAddresses($registrationRequest, $domain) {
        foreach ($registrationRequest->nameservers as $i => $nameserver) {
            if (!Utility::isGlueRequired($domain, $nameserver->name)) {
