...
if ($model->errorDisplayMessage == '') {
    ...
    foreach ($model->viewDataGrouped as $group => $grouppagedata) {
        ...
        $w = $this->widget('application.widgets.jqGrid.jqGridWidget', array('gridId' => 'emailDisabler',
            'caption' => $group,
            'thisfile' => $this->createUrl('emailDisabler/menu', array('model' => $model_serialised)),
            'datatype' => 'jsonstring','datastr' => $datastr,'model' => $model,'selections' => false,
            'tableId' => 'thisJqGrid' . $groupIndex,'numrows' => $hugeNumRows,'navGrid' => false,'pager' => false,
            'filterBar' => false));
        $groupIndex += 1;
        echo '<br/>';
        echo '<br/>';
