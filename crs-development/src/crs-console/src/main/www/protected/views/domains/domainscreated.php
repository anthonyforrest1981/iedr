...
<table>
    ...
    foreach ($model->domainsprocessed as $domain => $result) {
        ...
        if (isset($result['error'])) {
            ...
            $returnUrl = 'Card declined.' . CHtml::link("Click here to try again/another card.", Yii::app()->createUrl('domains/domaindetails', array(
                'errorReturnData' => $modelSerialized)));
            echo '<td>' . CHtml::encode($domain) . '</td><td>' . WSAPIError::getErrorsNotEmpty($result['error'], $returnUrl) . '</td>';
        } else {
    ...
    <tr>
        ...
        if ($uploaderResult->hasErrors()) {
            if (isset($uploaderResult->model['error'])) {
                ...
                foreach ($uploaderResult->getHumanReadableResults() as $docResult) {
