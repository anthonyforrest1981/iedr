...
class AllTicketsModel extends GridModelBase {
    ...
    public function __construct() {
        $this->columns = array(
            'A' => array('resultfield' => 'id','criteriafield' => null,'label' => 'Ticket','width' => 35,
                'type' => 'int'),
            'B' => array('resultfield' => 'domainName','criteriafield' => 'domainName','label' => 'Domain',
                'width' => 90,'type' => 'string'),
            'C' => array('resultfield' => 'domainHolder','criteriafield' => 'domainHolder','label' => 'Holder',
                'width' => 90,'type' => 'string'),
            'D' => array('resultfield' => 'adminStatus','criteriafield' => 'adminStatus','label' => 'Admin Status',
                'width' => 60,'wildcardpadding' => 'NONE',
                'selectlist' => $this->arrayAsColModelSelectOptions(GridUtility::getAdminStatuses())),
            'E' => array('resultfield' => 'techStatus','criteriafield' => 'techStatus','label' => 'Tech Status',
                'width' => 30,'wildcardpadding' => 'NONE',
                'selectlist' => $this->arrayAsColModelSelectOptions(GridUtility::getTechStatuses())),
            'F' => array('resultfield' => 'financialStatus','criteriafield' => 'financialStatus',
                'label' => 'Financial Status','width' => 40,'wildcardpadding' => 'NONE',
                'selectlist' => $this->arrayAsColModelSelectOptions(GridUtility::getFinancialStatuses())),
            'G' => array('resultfield' => 'type','criteriafield' => 'type','label' => 'Ticket Type','width' => 30,
                'wildcardpadding' => 'NONE',
                'selectlist' => $this->arrayAsColModelSelectOptions(GridUtility::getTicketTypes())),
            'H' => array('resultfield' => '?','criteriafield' => null,'label' => 'Expiry Date','width' => 40,
                'type' => 'date'));
    }
    ...
    public function getSearchBase() {
        ...
        $criteria->customerStatus = CRSTicketAppService_customerStatus::_NEW;
        $criteria->to = date('Ymd');
        if (isset($this->days))
            $this->days = Utility::mb_trim($this->days);
        ...
        $tm = time();
        Yii::log("DAYS= " . print_r($this->days, true));
        $criteria->from = date('Y-m-d', $tm - (86400 * $this->days)) . 'T00:00:00';
     ...
     *
     * @param CRSTicketAppService_ticketVO $t
     *            Ticket object
     * @return array array-format fromatted ticket data
     * @access public
     */
    public function addResults($t) {
        $ticket_expiry_date = Utility::getTicketExpirationDate($t->creationDate);
        $ticket_holder = '?';
        ...
        return array('A' => $t->id,'B' => $this->createLink(encode($t->domainName), $t->id),
            'C' => encode($ticket_holder),'D' => $t->adminStatus,'E' => $t->techStatus,'F' => $t->financialStatus,
            'G' => $t->type,'H' => $ticket_expiry_date);
    }
