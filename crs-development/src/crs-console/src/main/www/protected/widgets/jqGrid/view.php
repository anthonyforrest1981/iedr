...
$gridId = isset($this->gridId) ? $this->gridId : $tableId;
$cookieNumrowId = '"numrows_' . $gridId . '"'; // string of literal javascript string, i.e. "numrows_..."
$width = $this->width;
...
if ($this->model != null) {
    ...
    foreach ($this->model->columns as $k => $v) {
        ...
        if (isset($v['sorting'])) {
            $shouldSort = $v['sorting'];
        }
        $sort = 'sortable:' . ($shouldSort ? 'true, sorttype:"' . (isset($v['type']) ? $v['type'] : 'string') . '"' : 'false');
        $format = '';
        if (isset($v['link'])) {
...
<script type="text/javascript">
   ...
   function getNumGridRows() {
       ...
       var cookieId = <?php echo $cookieNumrowId; ?>;
       if ("<?php echo $navGrid; ?>" == "true"){
           var cookienumrows = getCookie(cookieId);
           if (cookienumrows != null && cookienumrows>0) {
               numrows = cookienumrows;
           }
           setCookie(cookieId, numrows, 3);
       }
       return numrows;
   }
</script>
...
<script type="text/javascript">
   ...
   $(document).ready(function() {
      jQuery('#<?php echo $tableId; ?>').jqGrid({
         hideGrid: false,
         hoverrows: true,
         rowList:[5,10,15,20,25,50,100,250<?php
        if ($allowAll) {
