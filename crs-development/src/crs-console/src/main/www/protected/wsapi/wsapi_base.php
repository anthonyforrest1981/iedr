...
function pretty_print_xml($x) {
    ...
    if ($x != null and $x != '') {
        $xml->loadXML(preg_replace('/ {2,}/u', ' ', preg_replace('/[\r\n\t]+/u', ' ', $x)));
    }
    return $xml->saveXML();
}
 ...
 *
 * @param object $o
 *            object (e.g. a model) to add data to
 * @param string $n
 *            object public data member name
 * @param array $a
 *            object or array to add (e.g. an object returned by a soap call)
 * @param string $s
 *            separator used to construct target object data membernames
 * @return void
 */
function addToObjectAsProperties($o, $n, $a, $s = '_') {
    // NOTE this doesn't work for Yii objects which implement __set() !
    if (!is_object($o))
        die("addToObjectAsProperties: first param (object to argument) must be an object");
    if (!is_string($n))
        die("addToObjectAsProperties: second param (name of member) must be a string");
    if (!is_string($s))
        die("addToObjectAsProperties: optional fourth param (separator) must be a string");
    if (is_object($a))
...
class crs_wsapi_service_base {
     ...
     */
    static $crs_soap_options = array('soap_version' => SOAP_1_1,'exceptions' => true,'trace' => 1,
        'cache_wsdl' => WSDL_CACHE_MEMORY,'compression' => SOAP_COMPRESSION_ACCEPT);
    /**
     ...
     *
     */
    public static $log_quench = false;
     ...
     *            log message
     * @param string $f
     *            filename, typically set with "__FILE__" by caller
     * @param string $l
     *            line-number, typically set with "__LINE__" by caller
     * @return void
     * @access public
     * @static
     *
     ...
     *            array of possible exception names
    ...
    static function getMessageFromException($exception, $expected_list) {
        ...
        if (mb_stripos($msg, 'exception') === false) {
            ...
            foreach ($expected_list as $e)
                if (mb_stripos($msg, $e) !== false)
                    $newmsg .= $e . ',';
            if ($newmsg != null)
                $msg = $newmsg;
        }
        return $msg . ' : ' . CHtml::encode($exception->faultstring);
    }
    ...
    static function logrequest($client) {
        if ($client != null && !crs_wsapi_service_base::$log_quench) {
            if (is_object($client)) {
                $m = self::getSoapUrl() . "\n";
                $m .= $client->__getLastRequestHeaders() . "\n";
                $xml = pretty_print_xml($client->__getLastRequest());
                $xml = self::hideRestricted($xml);
                $m .= $xml . "\n";
    ...
    /**
     * Replaces restricted values within XML string with ######
     *
     * @param
     *            string
     * @return string
     ...
     *
     */
    static function hideRestricted($xml) {
        $restricted = array('cardHolderName','cardNumber','cardType','cardExpDate','cvnNumber','password','oldPassword',
            'newPassword','repeatedPassword','superUserPassword');
        $patterns = array();
        $replacements = array();
        foreach ($restricted as $keyWord) {
            $patterns[$keyWord] = '/<' . $keyWord . '>.*<\/' . $keyWord . '>/u';
