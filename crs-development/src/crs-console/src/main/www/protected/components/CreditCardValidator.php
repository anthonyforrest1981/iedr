...
class CreditCardValidator extends RegexValidator {
    ...
    public function __construct() {
        ...
        $this->regex .= '$/u';
        // remove final pipe
        $this->regex = str_replace(')|$/u', ')$/u', $this->regex);
     ...
     *
     * @param object $model
     *            model containing credit card data
     * @param string $attribute
     *            model attribute name containing credit cared number string
    ...
    protected function strip_non_numeric($cardno) {
        return preg_replace('/[^0-9]/u', null, $cardno);
    }
     ...
     *            credit card number
     * @param
     *            string &$cardtype optional type; if null, will set on return to the first type the card-number validated as
     ...
     */
    private static function CCLuhnCheck($str) {
        $odd = true;
        $sum = 0;
        foreach (array_reverse(Utility::mb_str_split($str)) as $num)
            $sum += array_sum(Utility::mb_str_split((string) (($odd = !$odd) ? $num * 2 : $num)));
     ...
     *            credit card number
     * @param
     *            string &$typ optional type ; if null, will be set to the type of the first regex the card number was found to match, if any
    ...
    public static function CCPrefixLengthTypeCheck($num, &$typ = null) {
        ...
        if ($typ != null and isset(CreditCardValidator::$card_patterns[$typ])) {
            $pass = preg_match('/^' . CreditCardValidator::$card_patterns[$typ] . '$/u', $num);
        } else {
