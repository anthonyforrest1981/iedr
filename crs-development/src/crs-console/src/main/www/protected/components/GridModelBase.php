 ...
 */
class GridModelBase extends ReturningModelBase {
    ...
    var $columns = array(
        'A' => array('resultfield' => 'name','criteriafield' => 'domainName','label' => 'Domain Name','width' => 60,
            'type' => 'string','link' => '../site/viewsomething/'));
    /**
     *
     ...
     *            created by {@link getDataReq} from request data
    ...
    public function getSort($sortparams) {
        ...
        $sort->ascending = (mb_strtolower($sortparams['ord']) === 'desc') ? false : true;
        $sortByColumn = $this->columns[$sortparams['col']];
        if (isset($sortByColumn['sortby'])) {
            $sort->sortBy = $sortByColumn['sortby'];
        } else {
            $sort->sortBy = $sortByColumn['resultfield'];
        }
     ...
     *
     * @param array $arr
     *            array of key-value pairs for a select-list based search
    ...
    /**
     * get base search criteria object for use in backend requests (to be overridden)
     */
     ...
     *
     * CRSDomainAppService_plainDomainSearchCriteriaVO Object
     * (
     * ...
     * [domainName] => blahblah
     * ...
     ...
     *            deserialised jqGrid search filter
        ...
        foreach ($searchparams as $k => $r) {
            ...
            if (is_object($r) and isset($r->field)) {
                ...
                if ($f != null) {
                    $d = $r->data;
                    try {
                        $d = Utf8Validator::validateAndNormalizeUtf8($d);
                    } catch (Exception $e) {
                        throw new Exception($e->getMessage() . ' within search criteria for field: ' . print_r($f, true));
                    }
