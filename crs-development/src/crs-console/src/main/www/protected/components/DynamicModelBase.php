 ...
 */
class DynamicModelBase extends FormModelBase {
    /**
     * validation rules
     *
     * @var array
     ...
     *
     * @param
     *            {@link FormModelBase} $model other model to add
     * @return void
     * @access public
     ...
     *
     * @param
     *            {@link DynamicModelBase} $o object to copy from
     * @return void
     * @access public
     * @see __set
     */
     ...
     *            name of attribute having a validation rule
     * @param string $r
     *            validation rule name (defaults to 'safe')
     * @return void
     * @access protected
    ...
    protected function addValRule($varname, $r = 'safe') {
        ...
        if ($this->val_rules == null or !is_array($this->val_rules)) {
            throw new CException(Yii::t('yii', 'DynamicModelBase: Property "{class}.{property}" (set in ctor) is null or not an array.', array(
                '{class}' => get_class($this),'{property}' => 'val_rules')));
        }
    ...
    public function __get($name) {
        ...
        $getter = 'get' . $name;
        $names_lowercase_prefix = mb_substr(mb_strtolower($name), 0, 2);
        if (method_exists($this, $getter))
            return $this->$getter();
        else if (strcmp($names_lowercase_prefix, 'on') === 0 && method_exists($this, $name)) {
