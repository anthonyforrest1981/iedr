 ...
 */
class ReturningController extends AuthOnlyBaseController {
     ...
     *
     * @param unknown $selectionID
     *            html unique class ID of the selected jqGrid's cell containing the primary-key value of the selected Row,
     * @param unknown $buttonLabel
     *            unique html button label, to identify submit-button to un-disable
     * @return void
     * @access public
     */
...
EOF1;
     ...
     *
     * @param object $model
     *            the model containing any data to be rendered by the calling view
     * @param string $selectionID
     *            html tag ID of hidden form field to hold returned value
    ...
    public function outputReturningFormHtml($model, $selectionID, $buttonLabel = 'Return to Form') {
        if (is_array($model->returningData) and isset($model->returningData['returnurl']) and $model->returningData['returnurl'] and mb_strlen($model->returningData['returnurl']) > 0) {
            ...
            $returnurl = self::whitelistReturnUrl($model->returningData['returnurl']);
            $form = $this->beginWidget('CActiveForm', array('action' => $returnurl));
