...
public class PhoneValidator extends FieldValidatorSupport {
    ...
    public void validate(Object object) throws ValidationException {
        ...
        if (fieldValue == null || ((PhoneWrapper) fieldValue).getCurrentList() == null
                || ((PhoneWrapper) fieldValue).getCurrentList().size() == 0) {
            addFieldError(getFieldName(), object);
        } else {
            for (int i = 0; i < ((PhoneWrapper) fieldValue).getCurrentList().size(); i++) {
                String value = ((PhoneWrapper) fieldValue).getCurrentList().get(i) == null ? null
                        : ((PhoneWrapper) fieldValue).getCurrentList().get(i);
