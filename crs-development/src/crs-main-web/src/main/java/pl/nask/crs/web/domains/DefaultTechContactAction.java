...
public class DefaultTechContactAction extends AuthenticatedUserAwareAction {
    ...
    public DefaultTechContactAction(NicHandleSearchService nicHandleSearchService,
            ContactSearchService contactSearchService, AccountSearchService accountSearchService) {
    ...
    public void defaultTechContact() throws NicHandleNotFoundException {
        ...
        Account account;
        try {
            account = accountSearchService.getAccount(Long.parseLong(domainWrapper.getAccountNo()));
        } catch (Exception e) {
            throw new IllegalArgumentException("Wrong account number");
        }
        Validator
                .assertNotNull(account.getBillingContact(), "Billing contact for account " + account + " is undefined");
