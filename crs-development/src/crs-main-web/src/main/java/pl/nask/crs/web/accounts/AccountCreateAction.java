...
public class AccountCreateAction extends AbstractAccountAction {
    ...
    public AccountCreateAction(AccountAppService accountAppService, CountryFactory countryFactory,
            PaymentAppService paymentAppService) {
        super(accountAppService, countryFactory, paymentAppService);
    }
    ...
    public String create() throws Exception {
        try {
            setStatuses(Arrays.asList(AccountStatus.values()));
            ...
            if (!createAccountContener.isAccessGroups())
                addActionError("The billing contact "
                        + createAccountContener.getAccount().getBillingContact().getNicHandle()
                        + " has no permission group assigned. Click the icon next to the billing contact to fix it.");
            ...
            addActionError("The nic handle " + ex.getNicHandleId() + " is already assigned to the account nr "
                    + ex.getAccountId());
            return ERROR;
