...
public class NicHandleViewAction extends AuthenticatedUserAwareAction {
    ...
    public NicHandleViewAction(NicHandleAppService nicHandleAppService, AccountSearchService accountSearchService,
    ...
    public String view() throws Exception {
        ...
        int limit = 15;
        LimitedSearchResult<HistoricalObject<NicHandle>> res = nicHandleAppService.history(getUser(), nicHandleId,
                (tableParams.getPage() - 1) * limit, limit);
        paginatedResult = new TicketsPaginatedList<HistoricalObject<NicHandle>>(res.getResults(),
                (int) res.getTotalResults(), tableParams.getPage(), limit);
    ...
    public String alterStatus() throws Exception {
        try {
            ...
            addActionError("Nic handle " + ex.getNicHandleId()
                    + " is a billing contact of an account. Its status cannot be changed to Deleted.");
            return ERROR;
        } catch (NicHandleIsTicketContactException ex) {
            addActionError("Nic handle " + ex.getNicHandleId()
                    + " is a ticket contact. Its status cannot be changed to Deleted.");
            return ERROR;
        }
    }
    ...
    public boolean hasNoAccessRecord() throws AccessDeniedException {
        return userAppService.getUser(getUser(), nicHandleId) == null;
    }
}
