 ...
 */
public class HistoricalDomainAction extends
        GenericSearchAction<HistoricalObject<Domain>, HistoricalDomainSearchCriteria> {
    ...
    private final static List<SortCriterion> DESCENDING_ORDER =
            Arrays.asList(new SortCriterion("changeDate", false), new SortCriterion("changeId", false));

    public HistoricalDomainAction(DomainAppService domainAppService,
            final HistoricalDomainService historicalDomainService, AccountSearchService accountSearchService,
    ...
    public HistoricalDomainAction(DomainAppService domainAppService,
        ...
        super(new AppSearchService<HistoricalObject<Domain>, HistoricalDomainSearchCriteria>() {
            public LimitedSearchResult<HistoricalObject<Domain>> search(AuthenticatedUser user,
                    HistoricalDomainSearchCriteria criteria, long offset, long limit, List<SortCriterion> orderBy)
    ...
    private boolean findDomain(long changeId, TicketsPaginatedList<HistoricalObject<Domain>> pResult) throws Exception {
        for (int i = 0; i < pResult.getList().size(); i++) {
            HistoricalObject<Domain> histDomain = (HistoricalObject<Domain>) pResult.getList().get(i);
