...
<body>
    ...
    <n:group2 titleText="Current deposit status">
        ...
        <n:field2 label="Available funds">
            <s:property value="getText('struts.money.format',{deposit.closeBalMinusReservations})"/>
        </n:field2>
    </n:group2>
    ...
    <s:form name="account_form" theme="simple">
        <s:hidden name="id"/>
        <center>
            <s:if test="%{hasPermission('account.deposit.correct.button')}">
                <s:submit value="Correct" action="deposit-correct-input" theme="simple"/>
            </s:if>
            <s:if test="%{hasPermission('account.deposit.topup.button')}">
