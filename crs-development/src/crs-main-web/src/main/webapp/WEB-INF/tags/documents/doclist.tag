...
<%@ attribute name="buyRequestPreviousAction" type="java.lang.String" required="false" %>
<n:group2 titleText="Results" cssIcon="group-icon-document">
    ...
    <display:table name="paginatedResult" id="documentRow" class="result" requestURI="" sort="external" excludedParams="resetSearch">
        ...
        <s:if test="%{#attr.showFilename==null || !#attr.showFilename}">
            ...
            <display:column title="BuyRequest" class="documents-name" sortable="true" sortProperty="buyRequestId">
                <s:if test="%{#attr.documentRow.buyRequest != null}">
                    ...
                    <s:a href="%{buyRequestUrl}">
                        ${attr.documentRow.buyRequest.id} (${attr.documentRow.buyRequest.domainName} by ${attr.documentRow.buyRequest.domainHolder})
                    </s:a>
                </s:if>
            </display:column>
        </s:if>
