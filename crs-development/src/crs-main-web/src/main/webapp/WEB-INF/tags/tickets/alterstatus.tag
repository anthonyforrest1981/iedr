...
<div id="<%=dialogId%>" class="jqmWindow">
    <n:group2 titleText="Alter Status" cssIcon="group-icon-status">
        ...
        <form action="ticketbrowser-alterStatus.action" method="post">
            ...
            <c:set var="adminStatus" value="${ticket.adminStatus}"/>
            <center>
                <s:select list="adminStatuses" name="newAdminStatus" theme="simple" listValue="description"
                             value="#attr.adminStatus" size="8"/>
            </center>
            <hr class="buttons"/>
            <center>
