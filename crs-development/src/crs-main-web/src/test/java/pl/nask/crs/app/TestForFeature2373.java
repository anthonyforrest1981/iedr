...
public class TestForFeature2373 extends AbstractCrsTest {
    ...
    public void testFlagsInTicket() throws Exception {
        ...
        AssertJUnit
                .assertFalse(t.getTicket().getOperation().getResellerAccountField().getNewValue().isAgreementSigned());
        ...
        AssertJUnit
                .assertTrue(t.getTicket().getOperation().getResellerAccountField().getNewValue().isAgreementSigned());
    }
    ...
    public void testFlagsInNicHandleHist() throws Exception {
        ...
        AssertJUnit.assertTrue(nh.getAccount().isAgreementSigned());
        AssertJUnit.assertTrue(nh.getAccount().isTicketEdit());
        LimitedSearchResult<HistoricalObject<NicHandle>> hist =
                nicHandleAppService.history(technicalLead(), nhid, 0, 10);
        AssertJUnit.assertEquals(1, hist.getTotalResults());

        nicHandleAppService.modifyNicHandle(technicalLead(), nhid, nh.getAccount().getId(),
                NicHandleTestHelp.createNewNicHandle(nh), "aaa");
        ...
        AssertJUnit.assertEquals(2, hist.getTotalResults());
    ...
    public void testFlagsInTicketHist() throws Exception {
        ...
        AssertJUnit.assertEquals(1, hist.getHistory().size());

        ticketAppService.checkOut(technicalLead(), ticketId);
        ticketAppService
                .updateAsAdmin(technicalLead(), ticketId, new TicketEdit(ticket.getTicket().getOperation()), null,
                        " asd ", false, false);

        // ticket saved, history record generated. both flags should be true.
        hist = ticketAppService.history(technicalLead(), ticketId);
        AssertJUnit.assertEquals(3, hist.getHistory().size());
    ...
    public void testTicketEditOff() throws Exception {
        ...
        AssertJUnit.assertFalse(t.getTicket().getOperation().getResellerAccountField().getNewValue().isTicketEdit());
        ticketAppService.checkOut(technicalLead(), 256750);
        ticketAppService.updateAsAdmin(technicalLead(), 256750, new TicketEdit(t.getTicket().getOperation()), null,
                "sth", false, false);
    }
    ...
    public void testTicketEditOn() throws Exception {
        ...
        AssertJUnit.assertTrue(t.getTicket().getOperation().getResellerAccountField().getNewValue().isTicketEdit());
        ticketAppService.checkOut(technicalLead(), 256745);
        ticketAppService.updateAsAdmin(technicalLead(), 256745, new TicketEdit(t.getTicket().getOperation()), null,
                "sth", false, false);
    }
    ...
    private AuthenticatedUser authenticate(String name) throws AuthenticationException {
